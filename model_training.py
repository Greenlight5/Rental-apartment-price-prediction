...
# Save train_medians to file
train_medians = {
    "monthly_arnona": df['monthly_arnona'].median(),
    "building_tax": df['building_tax'].median(),
    "total_floors": df['total_floors'].median(),
    "distance_from_center_normalized": df['distance_from_center_normalized'].median() if 'distance_from_center_normalized' in df.columns else 0,
    "distance_group": df['distance_group'].mode()[0] if 'distance_group' in df.columns else 1
}
